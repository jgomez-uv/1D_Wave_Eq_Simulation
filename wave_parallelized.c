...
void intrinsic_step(float *u_space_0,float *u_space_1, float *u_space_2, float C,int time_steps, int space_steps);
...
int main(){
	...
	int length_space = 10; // In meters
	int length_time = 20; // In seconds
	...
	FILE *output_file = fopen("output_wave_seq.csv","w");
	...
	// Time iterations onwards, sequential
	...
	// Reinisialization of variables, u_0, u_1 need to be reconfigured
	...
	// outputting on new file to compare:
	
	FILE *output_file_2 = fopen("output_wave_avx.csv","w");
	
	for (int i = 0; i<space_steps;i++){
		fprintf(output_file_2,"%f,",u_space_0[i]);
	}
	...
	for (int i = 0; i<space_steps;i++){
		fprintf(output_file_2,"%f,",u_space_1[i]);
	}
	fprintf(output_file_2,"\n");
	
	// Time iterations onwards, vectorized
	
	start_time = clock();
	
	for (int n = 2; n < time_steps; n++){
		intrinsic_step(u_space_0, u_space_1, u_space_2, C, time_steps, space_steps);

		if (n % ((time_steps)/res) == 0){
			for (int m = 0; m < space_steps;m++){
			fprintf(output_file_2,"%f,",u_space_2[m]);
			};
			fprintf(output_file,"\n");
		}
	...
	end_time = clock();
	float time_avx = ((float)(end_time - start_time) / CLOCKS_PER_SEC);
	
	float speedup = time_seq/time_avx;
	float efficiency = speedup/8.0*100.0;
	...
	fprintf(timing_file, "AVX time: %.6f seconds\n", time_avx);
	fprintf(timing_file,"Speedup: %.6f\n",speedup);
	fprintf(timing_file,"Efficiency: %.6f %%.\n",efficiency);
...
void intrinsic_step(float *u_space_0,float *u_space_1, float *u_space_2, float C,int time_steps, int space_steps){
	...
	for (int i = 2; i < space_steps - 1 - 8; i += 8){
		__m256 u_il 	= _mm256_loadu_ps(&u_space_1[i-1]);
		__m256 u_i 	= _mm256_loadu_ps(&u_space_1[i]);
		__m256 u_ir 	= _mm256_loadu_ps(&u_space_1[i+1]);
		__m256 u_ipre	= _mm256_loadu_ps(&u_space_0[i]);
		
		__m256 laplacian = _mm256_add_ps(u_il,u_ir);
		laplacian = _mm256_sub_ps(
			laplacian,
			_mm256_mul_ps(v2,u_i));
		
		__m256 result = _mm256_add_ps(
			_mm256_mul_ps(vm1,u_ipre),
			_mm256_mul_ps(v2,u_i));
		
		result = _mm256_add_ps(
			result,
			_mm256_mul_ps(vC,laplacian));
		
		_mm256_storeu_ps(&u_space_2[i],result);
	}
	
	for(int i = space_steps-9; i < space_steps-1;i++){
